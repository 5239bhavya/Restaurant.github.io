 ...
 *@package Presazine
 */
    $sr_content_type   = presazine_get_option( 'sr_content_type' );
    $number_of_sr_items = presazine_get_option( 'number_of_sr_items' );
    $slider_layout = presazine_get_option( 'slider_layout_option' );
    $slider_column = presazine_get_option( 'number_of_sr_column' );
    $slider_category = presazine_get_option( 'slider_category' );
    $enable_content     = presazine_get_option( 'slider_content_enable' );
    $enable_title     = presazine_get_option( 'slider_title_enable' );
    $enable_category     = presazine_get_option( 'slider_category_enable' );
    $enable_posted_on     = presazine_get_option( 'slider_posted_on_enable' );
    $slider_speed   = presazine_get_option( 'slider_speed' );
    $slider_dot   = presazine_get_option( 'slider_dot' );
    $slider_arrow   = presazine_get_option( 'slider_arrow_enable' );
    $slider_autoplay  = presazine_get_option( 'slider_autoplay_enable' );
    $slider_infinite  = presazine_get_option( 'slider_infinite_enable' );
    $slider_fade  = presazine_get_option( 'slider_fade_enable' );
    $image_overlay   = presazine_get_option( 'disable_white_overlay' );
    $header_font_size = presazine_get_option( 'slider_font_size');
    $slider_background_color = presazine_get_option( 'slider_background_color');
    $slider_content_position = presazine_get_option( 'slider_content_position_option');
    $slider_social_text = presazine_get_option( 'slider_social_title_text');
    $slider_alt_btn_text = presazine_get_option( 'slider_alt_custom_btn_text');
    $slider_alt_btn_url = presazine_get_option( 'slider_alt_custom_btn_url');
    $slider_social_link  = presazine_get_option( 'slider_social_link' );
    ...
    for( $j=1; $j<=$number_of_sr_items; $j++ ) :
        $featured_slider_page_posts[] = presazine_get_option( 'slider_page_'.$j );
        $featured_slider_posts[] = presazine_get_option( 'slider_post_'.$j );
    endfor;
    ?>
    <style>
                        ...
                        <div class="<?php echo esc_attr($class); ?> featured-content-wrapper">
                            <header class="entry-header">
                                ...
                                <?php if ($enable_title==true):
                                    $featured_slider_before_title = presazine_get_option( 'slider_title_meta_'.$j );
                                    $featured_slider_after_title = presazine_get_option( 'slider_after_title_'.$j );
                                 ?>
                            ...
                            <?php if ( ($enable_content==true)): ?>
                                <div class="entry-content">
                                    <?php
                                        $excerpt = presazine_the_excerpt( 30 );
                            ...
                            <?php if ( ($enable_posted_on==true)) { ?>
                                <div class="entry-meta">                 
                                    <?php presazine_posted_on(); ?>
                                </div><!-- .entry-meta -->
                            <?php } ?>
                            <?php
                            $readmore_text   = presazine_get_option( 'slider_custom_btn_text_' . $j ); 
