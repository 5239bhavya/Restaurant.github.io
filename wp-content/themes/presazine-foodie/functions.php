...
if ( ! function_exists( 'presazine_foodie_enqueue_styles' ) ) :
    /**
     * Load assets.
     *
     * @since 1.0.0
     */
    function presazine_foodie_enqueue_styles() {
        wp_enqueue_style( 'presazine-style-parent', get_template_directory_uri() . '/style.css' );
        wp_enqueue_style( 'presazine-foodie-style', get_stylesheet_directory_uri() . '/style.css', array( 'presazine-style-parent' ), '1.0.0' );
    }
endif;
add_action( 'wp_enqueue_scripts', 'presazine_foodie_enqueue_styles', 99 );

/**
 * Register custom fonts.
 */
function presazine_foodie_fonts_url() {
    $fonts_url = '';
    $fonts     = array();
    ...
    if ( 'off' !== _x( 'on', 'Lota font: on or off', 'presazine-foodie' ) ) {
        $fonts[] = 'Lota';
    }
    ...
    if ( 'off' !== _x( 'on', 'Marck Script font: on or off', 'presazine-foodie' ) ) {
        $fonts[] = 'Marck Script';
    }
...
if ( ! function_exists( 'presazine_foodie_get_default_theme_options' ) ) :
     ...
     */
function presazine_foodie_get_default_theme_options() {
    ...
    $defaults['show_header_contact_info']   = false;
        ...
        $defaults['header_email']               = __( 'info@sensationaltheme.com','presazine-foodie' );
        $defaults['header_phone' ]              = __( '+1-541-754-3010','presazine-foodie' );
        $defaults['header_location' ]           = __( 'London, UK','presazine-foodie' );
        ...
        $defaults['homepage_design_layout_options']         = 'home-normal-blog';
        $defaults['homepage_sidebar_position']          = 'home-right-sidebar';
        
        // Headlines Section
        $defaults['disable_headlines_section']  = false;
        $defaults['headlines_title']            = esc_html__( 'Breaking News', 'presazine-foodie' );
        ...
        $defaults['slider_social_title_text']           = esc_html__( 'Follow Me:', 'presazine-foodie' );

        // Recent Section
        $defaults['disable_recent_section'] = false;
        $defaults['recent_title']           = esc_html__( 'Recent Posts', 'presazine-foodie' );
        ...
        $defaults['recent_see_all_txt']         = esc_html__( 'See All', 'presazine-foodie' );

        // Trending Section
        $defaults['disable_trending_section']   = false;
        $defaults['trending_title']         = esc_html__( 'Trending Posts', 'presazine-foodie' );
        ...
        $defaults['popular_title']              = esc_html__( 'Popular Posts', 'presazine-foodie' );
        ...
        $defaults['message_title']      = esc_html__( 'Hello, I am Nijasi Thitak', 'presazine-foodie' );
        $defaults['message_description']        = esc_html__( 'I’ve been working with a company this month doing blogger outreach for a project. Part of my job is to vet blogs and determine their audience, their traffic, and whether they’re a good fit for this particular project. Having spent several hours reviewing blogs in several markets, I’ve come to a conclusion: We all need to work on our About pages.', 'presazine-foodie' );
        $defaults['message_button_label']           = esc_html__( 'Know More', 'presazine-foodie' );
        ...
        $defaults['latest_posts_title']         = esc_html__( 'My Latest Stories', 'presazine-foodie' );
        ...
        $defaults['about_title']                = esc_html__( 'Recent Posts', 'presazine-foodie' );
        ...
        $defaults['team_title']             = esc_html__( 'Our Team Members', 'presazine-foodie' );
        ...
        $defaults['mustread_title']             = esc_html__( 'Must Read Posts', 'presazine-foodie' );
        $defaults['number_of_mustread_items']           = 3;
        $defaults['number_of_mustread_column']          = 3;
        $defaults['mustread_excerpt_length']            = 15;
        ...
        $defaults['mustread_see_all_txt']           = esc_html__( 'See All', 'presazine-foodie' );

        //Galleryview Section
        $defaults['disable_galleryview_section']    = false;
        $defaults['galleryview_title']              = esc_html__( 'Featured Posts', 'presazine-foodie' );
        ...
        $defaults['galleryview_see_all_txt']            = esc_html__( 'See All', 'presazine-foodie' );

        //Travel Section
        $defaults['disable_travel_section'] = false;
        $defaults['travel_title']               = esc_html__( 'Popular Posts', 'presazine-foodie' );
        ...
        $defaults['travel_see_all_txt']         = esc_html__( 'See All', 'presazine-foodie' );

        // Our Service Section
        $defaults['disable_services_section']   = false;
        $defaults['services_title']             = esc_html__( 'Reasons to Choose My Services', 'presazine-foodie' );
        $defaults['services_subtitle']          = esc_html__( 'Services', 'presazine-foodie' );
        ...
        $defaults['testimonial_title']              = esc_html__( 'Our clients reviews.', 'presazine-foodie' );
        $defaults['testimonial_viewall_text']               = esc_html__( 'View All Projects', 'presazine-foodie' );
        $defaults['testimonial_subtitle']           = esc_html__( 'Testimonials','presazine-foodie' );
        ...
        $defaults['cta_small_description']      = esc_html__( 'Need More Help?', 'presazine-foodie' );
        $defaults['cta_description']            = esc_html__( 'A better way to hire our best help', 'presazine-foodie' );
        $defaults['cta_button_label']           = esc_html__( 'Purchase Now', 'presazine-foodie' );
        $defaults['cta_button_url']             = '#';
        $defaults['cta_alt_button_label']       = esc_html__( 'Contact Us', 'presazine-foodie' );
        ...
        //General Section
        $defaults['latest_readmore_text']               = esc_html__('Read More','presazine-foodie');
        ...
        $defaults['copyright_text']             = esc_html__( 'Copyright &copy; All rights reserved.', 'presazine-foodie' );
        $defaults['powered_by_text']            = esc_html__( 'Presazine by Sensational Theme', 'presazine-foodie' );


    return $defaults;
}
endif;
add_filter( 'presazine_filter_default_theme_options', 'presazine_foodie_get_default_theme_options', 99 );
